...
public:
	...
	//-------------------------------------------------------------
	TYPE get(unsigned row,   // Which row.
	...
	//-------------------------------------------------------------
	Matrix getSubMatrix(unsigned startRow, unsigned endRow,
	...
	void reducedRowEcholon() {
		...
		for (unsigned rowIndex = 0; rowIndex < rows; ++rowIndex) {
			...
			if (ZERO != divisor) {
				...
				for (unsigned subRowIndex = (rowIndex + 1); subRowIndex < rows;
						++subRowIndex) {
		...
		for (unsigned rowIndex = (rows - 1); rowIndex > 0; --rowIndex) {
			unsigned row = order[rowIndex];
			unsigned column = getLeadingZeros(row);
			for (unsigned subRowIndex = 0; subRowIndex < rowIndex;
					++subRowIndex) {
				unsigned subRow = order[subRowIndex];
				rowOperation(subRow, row, -matrix[subRow][column]);
			}
	...
	Matrix const getInverse() const {
		...
		Matrix inverseMatrix(*this, IdentityMatrix<TYPE>(rows, columns),
				TO_RIGHT);
		...
		Matrix result(
				inverseMatrix.getSubMatrix(0, rows - 1, columns,
						columns + columns - 1, inverseMatrix.order));
		...
		for (unsigned row = 0; row < rows; ++row)
			for (unsigned column = 0; column < otherMatrix.columns; ++column) {
				...
				for (unsigned index = 0; index < columns; ++index)
					result.matrix[row][column] += matrix[row][index]
	...
	//-------------------------------------------------------------
	Matrix const & operator *=(TYPE const & scalar) {
		*this = *this * scalar;
		return *this;
	}
	...
	//    matrix = data;
	//-------------------------------------------------------------
	Matrix & operator =(TYPE const * data) {
		unsigned index = 0;
	...
	Matrix(unsigned rowsParameter, unsigned columnsParameter) :
			rows(rowsParameter), columns(columnsParameter) {
		TYPE const ZERO = static_cast<TYPE>(0);
	...
	// row/column.
	//-------------------------------------------------------------
	Matrix(Matrix const & copyMatrix, unsigned omittedRow = INT_MAX,
	...
	//   A = [B | C]
	//-------------------------------------------------------------
	Matrix(Matrix const & copyMatrixA, Matrix const & copyMatrixB,
